...
JOINT_NAME_WHEEL_L = "l_wheel_joint"   # must match URDF xacro insert_wheel side="l"/"r"
JOINT_NAME_WHEEL_R = "r_wheel_joint"
...
class Pinky(Node):
    def __init__(self):
        ...
        self.declare_parameter('publish_tf', True)        # set False when an EKF owns odom->base_footprint
        self.declare_parameter('cmd_vel_timeout', 0.5)    # [s] stop motors if no cmd_vel within this window
    ...
    def update_and_publish(self):
        ...
        # cmd_vel watchdog: zero the motors if no command arrived within the timeout,
        # so the robot stops instead of holding the last velocity when a commander dies.
        if (current_time - self.last_cmd_vel_time).nanoseconds / 1e9 > self.cmd_vel_timeout:
